 ...
 *       'hi' (문자열)
 *       7 (숫자)
 * 2. 문장(statement: 진술, 선언)
 *   : 명령, 문장 끝에는 세미콜론(;)을 붙이거나 줄바꿈을 넣어서 문장의 종결을 구분
 *   : 하나 이상의 표현식이 모이면 문장
 *   ex) if문, switch문 등등 문으로 끝나는 키워드들은 statement의 문장에서 나온 의미
 *
 * 3. 키워드 & 식별자
 *    키워드(예약어)
 ...
 *       괄호 안에는 실제 웹 브라우저 화면에 표시할 내용(html 태그 사용가능), 어떤 결괏값이 저장된 변수 삽입 가능
 *     ? console.log() 메소드
...
// 사용자로 부터 입력받은 내용과 변소를 섞어서 나타냄
let userName = prompt('이름을 입력하세요.');
document.write(`<h1>${userName}님, 반갑습니다.</h1>`);

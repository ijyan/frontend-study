 ...
 *
 * ! 토큰
 * : 사용자 인증 및 권한 부여에 사용되는 함호화된 문자열
 * : 서버에서 사용자 인증을 진행하고, 해당 사용자에 대한 세션 정보를 나타냄
 * ex) 사용자가 로그인 > 서버는 사용자의 인증 정보를 확인, 토큰 생성 > 생성된 토큰을 사용하여 이후의 요청에 자신을 인증(HTTP 헤더에 포함하여 서버로 전송)
 *
 * ! 쿠키
 * : 웹사이트가 사용자의 브라우저에 저장하는 작은 데이터 조각
 * : 사용자의 세션관리, 개인화 설정, 추적 등에 사용
 * ex) 사용자가 웹 사이트에 방문 > 사이트는 쿠키를 생성하여 브라우저에 저장 > 이후 사용자가 다시 해당 사이트에 방문 > 브라우저가 쿠키 정보를 사이트에 전달하여 사용자를 식별
 *
 * ? react-cookie
 * 1. 설치: npm i react-cookie
 * 2. 쿠키 생성
 *    : 로그인을 통한 사용자 내역을 저장
 *    - useCookies() 훅
...
export const useTokenService = () => {
  ...
  const setToken = (token: any) => {
    // setCookie('쿠키명', 실제 쿠키, {쿠키가 설정될 때 사용되는 경로});
    // 경로: 쿠키가 유효한 URL로 경로 결정

{
  ...
  const stringOutput = generic<string>('string generic');
  const numberOutput = generic<number>(500);
  ...
  const pairOutput = pair<string, number>('string type', 40);
  ...
  const resultFunc = genericFunc<boolean>([true, false, true]);
  ...
  const myIndentity: GenericInterface<number> = identity;
  ...
  const myGenricVar = new GenericClass<number>(0, function (x, y) {
    return x + y;
  });
  const myGenricStringVar = new GenericClass<string>('', function (x, y) {
    return `${x}, ${y}`;
  });

...
function fetchData(callback: (data: any) => void) {
  ...
  /*
    fetch(url - 요청을 보낼 주소)
      ...
      .catch(error => console.log(error)); // fetch에서 일어나는 오류 처리
   */
  // : HTTP 응답 객체를 반환
    ...
    .then((user) => {
      ...
      fetch(`https://jsonplaceholder.typicode.com/posts?userId=${userId}`)
        ...
        .then((posts) => {
          ...
          // 세 번째 요청: 사용자의 앨번 데이터를 가져오기
          fetch(`https://jsonplaceholder.typicode.com/albums?userId=${userId}`)
            .then((response) => response.json())
            .then((albums) => {
              console.log('사용자의 앨범: ', albums);
              // 모든 데이터가 준비되었을 때 콜백 함수 호출
...
function handelUserData(data: any) {
  console.log('가져온 전체 데이터: ', data);
}
// 사용자 ID 1에 대한 데이터 불러오기
fetchUserData(1, handelUserData);

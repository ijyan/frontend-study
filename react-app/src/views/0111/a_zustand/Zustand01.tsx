 ...
 *
 * ? 다른 상태 라이브러리와의 차이점
 * Zustand: 직관적인 API, 학습 곡선 낮음
 * Redux: 엄격한 데이터 흐름과 불변성 유지 가능, 공개된 소스코드 많음, 학습 곡선 어려움
...
// create 함수의 파라미터에 함수 형태로 state의 초기값 state를 변경하는 함수를 선언
// Zustand 에서는 set 함수를 사용하여 상태를 업데이트
...
function Zustand01() {
  ...
  // store에 있는 속성 중 사용할 속성을, 구조분해 할당으로 가져옴
  const { count, increment } = useStore();

  // store에 원하는 특정 부분만 선택하여 사용
  ...
  return (
    <div>
      ...
      <button onClick={otherIncrement} type="button">
        otherIncrement
      </button>
    </div>
  );

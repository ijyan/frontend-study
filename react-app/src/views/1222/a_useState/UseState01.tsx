...
/**
 * ! 컴포넌트 단위의 상태 관리: useState
 * : React의 Hook 중 하나, 가변적인 상태 관리를 하는 기능
 *
 * ? useState의 상태와 렌더링의 관계
 * >> useState를 사용하여 상태(state)를 설정
 * >> 해당 상티의 변경이 컴포넌트의 재렌더링을 유발
 * : React가 렌더링을 다시 시작하여 DOM을 업데이트
 *
 * ? 상태 변경의 비동기적 특성
 * : useState로 상태를 변경하는 작업은 비동기적으로 이루어짐
 * : 두 번째 인자인 set 설정 함수를 호출한 직후에 상태 값이 즉시 변경되지 않을 수도 있다.
 */

function UseState01() {
  // count: 현재 상태 값
  // setCount: 상태를 업데이트하는 함수
  // useState<number>(0): 상태의 초기값(숫자)
